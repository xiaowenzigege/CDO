 ...
 *
 */
public class CDOProtoFileEncoder extends MessageToByteEncoder<CDOFileMessage>{
	
	 private static Logger log=Logger.getLogger(CDOProtoFileEncoder.class);
	@Override
	protected void encode(ChannelHandlerContext ctx, CDOFileMessage msg,
	...
	private byte[] encodeCDOHeader(Header msgHeader,int bodyLength,List<File> files){	
		    ...
		    byte[] header = new byte[ProtoFileProtocol.PROTOCOL_LEN+8*fileCount];	
		    //cdo魔数
