 ...
 */
public class CDOProtoFileDecoder extends ByteToMessageDecoder {
	 private static Logger log=Logger.getLogger(CDOProtoFileEncoder.class);
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in,
			...
			while(in.readableBytes()>=headLen){
				...
				//创建对象
				message=new CDOFileMessage();
				Header header=new Header();
				header.setType(type);
				message.setHeader(header);	
				//CDO 消息类型需要特别处理，其余为心跳消息检测
				if(type==ProtoFileProtocol.TYPE_CDO){
	...
	private boolean checkMsgType(byte type){
		...
		boolean flag=false;
		switch (type) {
			case ProtoFileProtocol.TYPE_CDO:
			case ProtoFileProtocol.TYPE_HEARTBEAT_REQ:
			case ProtoFileProtocol.TYPE_HEARTBEAT_RES:
			case ProtoFileProtocol.TYPE_STOPLOCALServer:	
				flag=true;
				break;
			default:
				flag=false;
